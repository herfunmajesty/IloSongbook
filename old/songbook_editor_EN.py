...
What it does:
- opens a song .txt file,
- lets you edit the text in the app,
- shows metadata and detected chords,
- has buttons for inserting chords and common markers,
- generates an HTML preview using song.html,
- opens the preview in a browser.

Requirements:
- Python 3.x
- pip install jinja2
- song.html in the same folder as this script
...
# ============================================================
# CORE - songbook conversion logic
# ============================================================
...
def validate_song_text(text: str) -> List[str]:
    ...
    if level and level not in LEVEL_TO_BOOTSTRAP:
        warnings.append(f"Unexpected level: {level}. Expected: Easy / Medium / Hard")

    title = metadata.get("t", "")
    if title and "\n" in title:
        warnings.append("Title looks suspicious - check {t:...} metadata")

    # Prosty check HTML-owych markerów pomocniczych
    for tag in ["s", "d", "r", "grey"]:
    ...
    deprecated_patterns = {
        r"<small><i>": "Old small+italic HTML marker. Consider using <s>...</s>.",
        r"</i></small>": "Old small+italic closing marker. Consider using </s>.",
        r"<b class=\"mark": "Old mark/highlight marker. Consider using <info>, <r> or <s> depending on meaning.",
        r"<span class=\"mark": "Old mark/highlight marker. Consider using <info>, <r> or <s> depending on meaning.",
    }
    for pattern, msg in deprecated_patterns.items():
        if re.search(pattern, text):
            warnings.append(msg)

    if re.search(r"(?im)^\s*info\s*:", text):
        warnings.append("Plain 'info:' detected. Consider using <info>...</info>.")

    if re.search(r"\[[^\]]*\\[^\]]*\]", text):
        warnings.append("Backslash chord marker detected, e.g. [A\\]. Decide whether to migrate it to [A/] or another standard.")
...
def render_song_html(text: str, template_file: Path) -> str:
    if Environment is None or FileSystemLoader is None:
        raise RuntimeError("Missing jinja2 library. Install it with: py -m pip install jinja2")

    parsed = parse_song(text)
    meta = parsed.metadata

    title = meta.get("t", "PREVIEW - missing title")
...
class SongbookEditor(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Radiobutton(chord_controls, text="[A/] ...", variable=self.chord_mode, value="slashdots").pack(side="left")
        ...
        ttk.Button(manual, text="Set", command=self.set_manual_chords).grid(row=0, column=1, padx=(4, 0))

        self.chord_buttons_frame = ttk.Frame(chord_box)
        self.chord_buttons_frame.grid(row=2, column=0, sticky="ew")

        snippets_box = ttk.LabelFrame(side, text="Snippets / markers", padding=8)
        ...
        ttk.Button(transform_box, text="Wrap <info>...</info>", command=lambda: self.wrap_selection("<info>\n", "\n</info>")).grid(row=5, column=0, sticky="ew", pady=2)
        ttk.Button(transform_box, text="Wrap <ex>...</ex>", command=lambda: self.wrap_selection("<ex>", "</ex>")).grid(row=6, column=0, sticky="ew", pady=2)
    ...
    def choose_template(self) -> None:
        ...
        self.status_var.set(f"Selected template: {self.template_file.name}")
        ...
        try:
            if not self.template_file.exists():
                messagebox.showerror(
                    "Missing template",
                    f"Template not found:\n{self.template_file}\n\nPut song.html next to the script or choose it with the button.",
                )
                return
            ...
            self.status_var.set(f"Preview generated: {self.preview_file}")
            self.refresh_analysis()
        except Exception as exc:
            messagebox.showerror("Preview error", str(exc))
            self.status_var.set("Error while generating preview")
    ...
    def refresh_analysis(self) -> None:
        ...
        self._populate_chord_buttons(parsed.chords)
    ...
    def _populate_chord_buttons(self, chords: List[str]) -> None:
        ...
        if not chords:
            ttk.Label(self.chord_buttons_frame, text="No chords detected").grid(row=0, column=0, sticky="w")
            return
    ...
    def set_manual_chords(self) -> None:
        ...
        chords = [c.strip() for c in re.split(r"[,;\s]+", raw) if c.strip()]
        self._populate_chord_buttons(chords)
        self.status_var.set("Setiono ręczny zestaw akordów")
