...
REPLACEMENTS = {
    ...
    "</s>": "</i></small>",
    "\n": "<br>",
    "<info>": "<div class=alert alert-warning mb-1>",
    "</info>": "</div>",
}
...
class SongbookEditor(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(preview_top, text="Uproszczony podgląd", font=("Segoe UI", 11, "bold")).grid(row=0, column=0, sticky="w")
        ttk.Button(preview_top, text="Odśwież", command=self.refresh_analysis).grid(row=0, column=1, sticky="e")
        ...
        for idx, key in enumerate(["t", "artist", "level", "d", "spotify", "youtube"]):
    ...
    def highlight_syntax(self) -> None:
        ...
        apply_regex(r"</?(?:s|d|r|grey)>", "custom_tag")
    ...
    def _strip_inline_formatting_for_preview(self, line: str) -> tuple[str, bool]:
        ...
        if re.search(r"<s>|<d>|<r>|<grey>", line, flags=re.IGNORECASE):
            is_section = True
    ...
    def _update_simplified_preview(self, parsed: ParsedSong) -> None:
        ...
        if not raw_lyrics.strip():
            self.preview_text.insert("end", "Brak treści piosenki.", "pv_warning")
        else:
    ...
    def set_manual_chords(self) -> None:
        ...
        self.status_var.set("Ustawiono ręczny zestaw akordów")
