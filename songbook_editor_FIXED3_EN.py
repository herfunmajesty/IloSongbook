...
def validate_song_text(text: str) -> List[str]:
    ...
    for key in REQUIRED_METADATA:
        if not metadata.get(key):
            warnings.append(f"Missing required metadata: {{{key}:...}}")
    ...
    for tag in ["s", "d", "r", "grey", "info", "ex"]:
        ...
        if open_count != close_count:
            warnings.append(f"Unclosed or extra marker: <{tag}> / </{tag}>")
...
def render_song_html(text: str, template_file: Path) -> str:
    ...
    title = meta.get("t", "PREVIEW - no title")
...
class SongbookEditor(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("ILO Songbook Editor - MVP EN")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(top, text="Open .txt", command=self.open_file).grid(row=0, column=0, padx=3)
        ttk.Button(top, text="Save", command=self.save_file).grid(row=0, column=1, padx=3)
        ttk.Button(top, text="Save as", command=self.save_file_as).grid(row=0, column=2, padx=3)
        ttk.Button(top, text="Preview HTML", command=self.preview_html).grid(row=0, column=3, padx=12)
        ttk.Button(top, text="Choose song.html", command=self.choose_template).grid(row=0, column=4, padx=3)
        ttk.Button(top, text="Refresh analysis", command=self.refresh_analysis).grid(row=0, column=5, padx=3)

        self.file_label = ttk.Label(top, text="No file")
        ...
        self._configure_text_highlighting()
        ...
        # Prawy panel - uproszczony podgląd
        preview_frame = ttk.Frame(main, padding=8)
        preview_frame.rowconfigure(1, weight=1)
        preview_frame.columnconfigure(0, weight=1)

        preview_top = ttk.Frame(preview_frame)
        preview_top.grid(row=0, column=0, sticky="ew", pady=(0, 6))
        preview_top.columnconfigure(0, weight=1)
        ttk.Label(preview_top, text="Simplified preview", font=("Segoe UI", 11, "bold")).grid(row=0, column=0, sticky="w")
        ttk.Button(preview_top, text="Refresh", command=self.refresh_analysis).grid(row=0, column=1, sticky="e")
        ...
        meta_box = ttk.LabelFrame(side, text="Metadata", padding=8)
        ...
        chord_box = ttk.LabelFrame(side, text="Song chords", padding=8)
        ...
        ttk.Label(chord_controls, text="Mode:").pack(side="left")
        ...
        ttk.Button(manual, text="Add", command=self.set_manual_chords).grid(row=0, column=1, padx=(4, 0))

        self.chord_buttons_frame = ttk.Frame(chord_box)
        self.chord_buttons_frame.grid(row=2, column=0, sticky="ew")

        snippets_box = ttk.LabelFrame(side, text="Formulas / markers", padding=8)
        snippets_box.grid(row=2, column=0, sticky="ew", pady=(0, 8))
        self._build_snippet_buttons(snippets_box)

        transform_box = ttk.LabelFrame(side, text="Selection transforms", padding=8)
        transform_box.grid(row=3, column=0, sticky="ew", pady=(0, 8))
        ttk.Button(transform_box, text="Chords → slash", command=self.selection_chords_to_slash).grid(row=0, column=0, sticky="ew", pady=2)
        ttk.Button(transform_box, text="Chords → normal", command=self.selection_chords_to_normal).grid(row=1, column=0, sticky="ew", pady=2)
        ttk.Button(transform_box, text="Wrap <s>...</s>", command=lambda: self.wrap_selection("<s>", "</s>")).grid(row=2, column=0, sticky="ew", pady=2)
        ttk.Button(transform_box, text="Wrap <r>...</r>", command=lambda: self.wrap_selection("<r>", "</r>")).grid(row=3, column=0, sticky="ew", pady=2)
        ttk.Button(transform_box, text="Wrap <d>...</d>", command=lambda: self.wrap_selection("<d>", "</d>")).grid(row=4, column=0, sticky="ew", pady=2)
        ttk.Button(transform_box, text="Hint / EX", command=lambda: self.insert_at_cursor("<ex></ex>")).grid(row=5, column=0, sticky="ew", pady=2)

        warnings_box = ttk.LabelFrame(side, text="Validation", padding=8)
    ...
    def _configure_text_highlighting(self) -> None:
        """Kolorowanie składni w edytorze TXT."""
        self.text.tag_configure("metadata", foreground="#138000")      # zielony
        self.text.tag_configure("chord", foreground="#c00000")         # czerwony
        self.text.tag_configure("custom_tag", foreground="#003a8c")    # granatowy
        self.text.tag_configure("html_tag", foreground="#0057b8")      # niebieski
        self.text.tag_configure("special", foreground="#7a3e9d")       # fioletowy, np. [stop]
        self._highlight_job = None

    def _configure_preview_tags(self) -> None:
        """Style dla uproszczonego podglądu po prawej stronie."""
        self.preview_text.tag_configure("pv_title", font=("Segoe UI", 15, "bold"), spacing3=6)
        self.preview_text.tag_configure("pv_artist", font=("Segoe UI", 11, "italic"), foreground="#555555", spacing3=8)
        self.preview_text.tag_configure("pv_meta", foreground="#138000", font=("Segoe UI", 9))
        self.preview_text.tag_configure("pv_section", foreground="#003a8c", font=("Segoe UI", 9, "italic"), spacing1=6, spacing3=3)
        self.preview_text.tag_configure("pv_chord", foreground="#c00000", font=("Segoe UI", 10, "bold"))
        self.preview_text.tag_configure("pv_warning", foreground="#a15c00", font=("Segoe UI", 9, "bold"))
        self.preview_text.tag_configure("pv_error", foreground="#b00020", font=("Segoe UI", 9, "bold"))

    def schedule_highlight(self) -> None:
        """Nie koloruj przy każdym znaku natychmiast, tylko z małym opóźnieniem."""
    ...
    def highlight_syntax(self) -> None:
        """Podświetla metadane, akordy i znaczniki w polu tekstowym."""
        self._highlight_job = None
        content = self.get_text()

        for tag in ("metadata", "chord", "custom_tag", "html_tag", "special"):
            self.text.tag_remove(tag, "1.0", "end")

        def apply_regex(pattern: str, tag: str) -> None:
        ...
        # Twoje customowe didaskalia i znaczniki formatowania
        apply_regex(r"</?(?:s|d|r|grey|info|ex)>", "custom_tag")

        # Typowe tagi HTML, jeżeli jeszcze występują w starych plikach, np. <small><i>
        apply_regex(r"</?[a-zA-Z][a-zA-Z0-9]*(?:\s+[^<>]*)?>", "html_tag")

        # Akordy i komendy w nawiasach kwadratowych
        for match in re.finditer(r"\[[^\]\n]+\]", content):
            value = match.group(0).lower()
            tag = "special" if any(x in value for x in ["stop", "muted", "riff", "back", "pause", "solo"]) else "chord"
    ...
    def open_file(self) -> None:
        ...
        self.manual_chords = []
        self.manual_chords_var.set("")
        self.text.edit_modified(False)
        self.file_label.configure(text=str(self.current_file))
        self.status_var.set(f"Opened file: {self.current_file.name}")
        self.refresh_analysis()
    ...
    def choose_template(self) -> None:
        path = filedialog.askopenfilename(
            title="Choose song.html template",
    ...
    def refresh_analysis(self) -> None:
        text = self.get_text()
        parsed = parse_song(text) if text.strip() else ParsedSong({}, "", [], ["No song content"])
        ...
        self._update_simplified_preview(parsed)

    def _strip_inline_formatting_for_preview(self, line: str) -> tuple[str, bool]:
        """Usuwa znaczniki techniczne z jednej linii i zwraca informację, czy to didaskalia."""
        is_section = False

        # Stare HTML-owe didaskalia, np. <small><i>(Chorus)</i></small>
        if re.search(r"<small>\s*<i>|<i>\s*<small>", line, flags=re.IGNORECASE):
            is_section = True

        # Twoje krótkie customowe znaczniki
        if re.search(r"<s>|<d>|<r>|<grey>|<info>|<ex>", line, flags=re.IGNORECASE):
            is_section = True

        # Czyścimy znaki formatowania, ale zostawiamy treść
        replacements = {
            "<s>": "", "</s>": "",
            "<d>": "", "</d>": "",
            "<r>": "", "</r>": "",
        ...
        # Awaryjnie usuń pozostałe tagi HTML, jeśli jakiś stary zapis się pojawi.
        result = re.sub(r"</?[a-zA-Z][a-zA-Z0-9]*(?:\s+[^<>]*)?>", "", result)
        return result, is_section

    def _insert_preview_line_with_chords(self, line: str, base_tag: Optional[str] = None) -> None:
        """Wstawia linię do uproszczonego podglądu i podświetla akordy."""
        pos = 0
        for match in re.finditer(r"\[[^\]\n]+\]", line):
            before = line[pos:match.start()]
            if before:
                self.preview_text.insert("end", before, base_tag if base_tag else ())
            self.preview_text.insert("end", match.group(0), "pv_chord")
            pos = match.end()
        rest = line[pos:]
        if rest:
            self.preview_text.insert("end", rest, base_tag if base_tag else ())
        self.preview_text.insert("end", "\n")

    def _update_simplified_preview(self, parsed: ParsedSong) -> None:
        """Odświeża trzecią kolumnę: uproszczony podgląd bez pełnego renderowania HTML."""
        ...
        self.preview_text.insert("end", f"{title}\n", "pv_title")
        if artist:
            self.preview_text.insert("end", f"{artist}\n", "pv_artist")
        self.preview_text.insert("end", f"Level: {level}    Duration: {duration} min\n", "pv_meta")
        self.preview_text.insert("end", f"Akordy: {', '.join(parsed.chords) if parsed.chords else '—'}\n", "pv_meta")
        ...
        self.preview_text.insert("end", "\n")
        self.preview_text.insert("end", "— podgląd tekstu —\n", "pv_meta")
        self.preview_text.insert("end", "\n")

        raw_lyrics = re.sub(r"\{.*?\}", "", self.get_text(), flags=re.DOTALL).strip("\n")
        if not raw_lyrics.strip():
            self.preview_text.insert("end", "No song content.", "pv_warning")
        else:
            # Nie robimy pełnego HTML; tylko czytelny podgląd: bez tagów formatowania, z czerwonymi akordami.
            ...
            for line in lines:
                if not line.strip():
                    ...
                    continue
                empty_count = 0
                clean_line, is_section = self._strip_inline_formatting_for_preview(line)
                self._insert_preview_line_with_chords(clean_line, "pv_section" if is_section else None)
    ...
    def _write_warnings(self, warnings: List[str], chords: List[str]) -> None:
        ...
        if not warnings:
            self.warnings.insert("end", "✔ OK - no obvious issues\n")
        else:
            for item in warnings:
                self.warnings.insert("end", f"❌ {item}\n")

        self.warnings.insert("end", "\n")
        self.warnings.insert("end", f"Detected chords: {', '.join(chords) if chords else '—'}\n")
        self.warnings.configure(state="disabled")
    ...
    def _populate_chord_buttons(self, chords: List[str]) -> None:
        ...
        if not chords:
            ttk.Label(self.chord_buttons_frame, text="No detected chords").grid(row=0, column=0, sticky="w")
            return
    ...
    def set_manual_chords(self) -> None:
        ...
        # "Ustaw" dodaje ręczne akordy do tych już wykrytych w tekście.
        ...
        self.status_var.set(
            f"Added manual chords: {', '.join(new_chords) if new_chords else '—'}"
        )
    ...
    def get_selection(self, show_message: bool = True) -> Optional[tuple[str, str, str]]:
        try:
            ...
            if show_message:
                messagebox.showinfo("No selection", "Select a text fragment first.")
            return None
    ...
    def _on_text_modified(self, _event=None) -> None:
        if self.text.edit_modified():
            self.status_var.set("Content changed - remember to save")
